...
# def draw_game(snake, ai_snake, letters, word, player_index, ai_index, p_score, ai_score, obstacles):
#     screen.fill(DARK)
...
#             if (new_head in snake[1:] or new_head in ai_snake or new_head in obstacles or
...
#                 player_score += 3
#                 ai_freeze_timer = 30  # Freeze AI for 3 seconds (30 frames at 10 FPS)
#                 word = get_random_word()
#                 player_index = 0
#                 ai_index = 0
#                 letters = spawn_letters(word, snake, ai_snake)
...
#             # AI movement only if not frozen
#             if ai_freeze_timer == 0 and letters and ai_index < len(word):
...
#                 else:
#                     ai_snake.pop()
#             elif ai_freeze_timer > 0:
#                 ai_freeze_timer -= 1

#             if (ai_snake[0] in ai_snake[1:] or ai_snake[0] in snake or ai_snake[0] in obstacles or
...
#             draw_game(snake, ai_snake, letters, word, player_index, ai_index, player_score, ai_score, obstacles)
#             clock.tick(FPS)
    ...
    while True:
        ...
        while running:
            ...
            if (new_head in snake[1:] or new_head in ai_snake or 
                any(new_head == (o[0], o[1]) for o in obstacles) or
                new_head[0] < 0 or new_head[0] >= WIDTH or new_head[1] < 0 or new_head[1] >= HEIGHT):
                running = False
                break
            ...
            for l in letters[:]:  # Create a copy to safely modify during iteration
                if (l[0], l[1]) == new_head and l[2] == word[player_index].upper():
                    pygame.mixer.Sound.play(EAT_SOUND)
                    player_index += 1
                    player_score += 1
            ...
            if (ai_snake[0] in ai_snake[1:] or ai_snake[0] in snake or 
                any(ai_snake[0] == (o[0], o[1]) for o in obstacles) or
                ai_snake[0][0] < 0 or ai_snake[0][0] >= WIDTH or ai_snake[0][1] < 0 or ai_snake[0][1] >= HEIGHT):
                running = False
                break
